...
def grid_search_scikit(ds, model, grid):
    ...
    # grid-search object
    clf = GridSearchCV(estimator=model, param_grid=grid, cv=10, scoring="accuracy")

    # train/fit the model
    clf.fit(X_train_val, y_train_val)
    # we do not use the train-validation split strategy since it is included in the CV procedure
...
best_par, best_score, y_test_pred, test_acc = grid_search_scikit(
    ds, lr, param_grid_test
)
