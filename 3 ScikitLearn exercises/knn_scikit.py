...
from sklearn.model_selection import (
    train_test_split,
)  # split train,validation and test st
...
### OUR EXAMPLE
# plot data
X, y = make_moons(n_samples=750, noise=0.3, random_state=123)
# dataset with random seed: X = 2D pts, y = layers

fig = plt.figure(figsize=(7, 5))
plt.plot(X[:, 0][y == 0], X[:, 1][y == 0], "r^")
# pts with y=0 are red (X[:,0] means first component, X[:,1] second)
plt.plot(X[:, 0][y == 1], X[:, 1][y == 1], "bs")  # pts with y=1 are blue
plt.show()

# execute the function
y_pred_train, y_pred_test, accuracy_values_train, accuracy_values_test = knn_scikit(
    X, y
)
