...
def perceptron_np_train(X_mat, y, theta, max_epochs):
    ...
    while num_epochs <= max_epochs and num_errors > 0:
        # we keep iterating over the data until we make no mistake or we reach the maximum number of iterations
...
def perceptron_np_homemade(X, y, max_epochs):
    ...
    theta = np.zeros(n + 1)
    # we consider the +1 since we need to insert the bias as well
